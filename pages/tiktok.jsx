import Image from "next/image";
import React from "react";
import TiktokImg from "../public/assets/projects/TiktokClone.PNG";
import { RiRadioButtonFill } from "react-icons/ri";
import Link from "next/link";

const tiktok = () => {
  return (
    <div className="w-full">
      <div className="w-screen h-[50vh] relative">
        <div className="absolute top-0 left-0 w-full h-[50vh] bg-black/70 z-10" />
        <Image
          className="absolute z-1"
          layout="fill"
          objectFit="cover"
          src={TiktokImg}
          alt="/"
        />
        <div className="absolute top-[70%] max-w-[1240px] w-full left-[50%] right-[50%] translate-x-[-50%] translate-y-[-50%] text-white z-10 p-2">
          <h2 className="py-2">Tiktok App</h2>
          <h3>Next JS / Tailwind / Sanity.io</h3>
        </div>
      </div>

      <div className="max-w-[1240px] mx-auto p-2 grid md:grid-cols-5 gap-8 py-8">
        <div className="col-span-4">
          <p>Project</p>
          <h2>Overview</h2>
          <p>
            I built this app to demonstrate knowledge working in Next JS and
            accessing API endpoints. This app was built using Next JS and is
            hosted on Sanity.io. This application supports user authentication
            using Sanity.io . Users are able to create a new account using their
            Google Account and can then sign in with it. You can be upload your
            video with many topics to the website and the website will store the
            videos you have uploaded using Sanity.io. You can search for videos
            that interest you by topic or by keywords. You can search different
            accounts to see what interesting videos they have posted on their
            wall. And you will also have a wall of your own home that you can
            post interesting videos on. You can interact with video content like
            liking or commenting, videos you've liked you can search for them
            again in the liked section in your wall.
          </p>
          <a
            href="https://github.com/minhhieu21193/tiktol-clone"
            target="_blank"
            rel="noreferrer"
          >
            <button className="px-8 py-2 mt-4 mr-8">Code</button>
          </a>
          <a
            href="https://tiktol-clone.vercel.app/"
            target="_blank"
            rel="noreferrer"
          >
            <button className="px-8 py-2 mt-4">Demo</button>
          </a>
        </div>
        <div className="col-span-4 md:col-span-1 shadow-xl shadow-gray-400 rounded-xl py-4">
          <div className="p-2">
            <p className="text-center font-bold pb-2">Technologies</p>
            <div className="grid grid-cols-3 md:grid-cols-1 ">
              <p className="text-gray-600 py-2 flex items-center">
                <RiRadioButtonFill className="pr-1" /> Next JS
              </p>
              <p className="text-gray-600 py-2 flex items-center">
                <RiRadioButtonFill className="pr-1" /> Tailwind
              </p>
              <p className="text-gray-600 py-2 flex items-center">
                <RiRadioButtonFill className="pr-1" /> TypeScript
              </p>
              <p className="text-gray-600 py-2 flex items-center">
                <RiRadioButtonFill className="pr-1" /> Sanity.io
              </p>
            </div>
          </div>
        </div>
        <Link href="/#projects">
          <p className="underline cursor-pointer">Back</p>
        </Link>
      </div>
    </div>
  );
};

export default tiktok;
